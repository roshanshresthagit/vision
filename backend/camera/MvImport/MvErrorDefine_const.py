...
MV_E_BUFOVER = (
    0x80000002  # < \~chinese 缓存已满                 \~english Buffer overflow
)
...
MV_E_ENCRYPT = (
    0x8000000E  # < \~chinese 加密错误                 \~english Encryption error
)
MV_E_OPENFILE = (
    0x8000000F  # < \~chinese 打开文件出现错误         \~english open file error
)
MV_E_BUF_IN_USE = (
    0x80000010  # < \~chinese 缓存地址已使用           \~english Buffer already in use
)
MV_E_BUF_INVALID = (
...
MV_E_PORT_IN_USE = (
    0x80000014  # < \~chinese 串口被占用               \~english Port is in use
)
...
MV_E_GC_GENERIC = (
    0x80000100  # < \~chinese 通用错误                 \~english General error
)
MV_E_GC_ARGUMENT = (
...
MV_E_GC_LOGICAL = (
    0x80000105  # < \~chinese 逻辑错误                 \~english Logical error
)
...
MV_E_USB_GENICAM = (
    0x80000303  # < \~chinese GenICam相关错误         \~english GenICam error
)
