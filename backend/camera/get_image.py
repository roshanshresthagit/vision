...
class CAMERA:
    ...
    def open_device(self, nConnectionNum):
        ...
        stdeviceList = cast(
            self.deviceList.pDeviceInfo[int(nConnectionNum)], POINTER(MV_CC_DEVICE_INFO)
        ).contents
    ...
    def start_grabbing(self):
        ...
        try:
            self.hThreadHandle = threading.Thread(
                target=self.work_thread, args=()
            )  # pass displayWidget id as argument to show the image
    ...
    def Color_numpy(self, data, nWidth, nHeight):
        data_ = np.frombuffer(
            data, count=int(nWidth * nHeight * 3), dtype=np.uint8, offset=0
        )
    ...
    def work_thread(self):
        ...
        while True:
            ...
            numArray = self.Color_numpy(
                img_buff, stOutFrame.stFrameInfo.nWidth, stOutFrame.stFrameInfo.nHeight
            )
